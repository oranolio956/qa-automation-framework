...
from schemas import (
    OrderRequest, OrderResponse, PaymentWebhook,
...
# OPA configuration
OPA_URL = get_secret('OPA_URL', 'http://localhost:8181')
OPA_ENABLED = get_secret('OPA_ENABLED', 'true').lower() == 'true'
OPA_FAIL_CLOSED = get_secret('OPA_FAIL_CLOSED', 'false').lower() == 'true'  # Fail-closed for production
OPA_TIMEOUT = get_int('OPA_TIMEOUT', 1)  # seconds
OPA_CIRCUIT_BREAKER_THRESHOLD = get_int('OPA_CIRCUIT_BREAKER_THRESHOLD', 5)
OPA_CIRCUIT_BREAKER_WINDOW = get_int('OPA_CIRCUIT_BREAKER_WINDOW', 300)  # 5 minutes
...
def start_opa_sidecar():
    """Start OPA sidecar process"""
    global opa_process
    try:
        if OPA_ENABLED:
            opa_command = [
                'opa', 'run', '--server',
                '--addr', '0.0.0.0:8181',
                '--config-file', 'infra/opa/config.yaml',
                'infra/opa/policies/'
            ]
            opa_process = subprocess.Popen(opa_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            logger.info("OPA sidecar started on port 8181")
            
            # Register cleanup on exit
            atexit.register(stop_opa_sidecar)
    except Exception as e:
        logger.warning(f"Failed to start OPA sidecar: {e}")
        
def stop_opa_sidecar():
    """Stop OPA sidecar process"""
    global opa_process
    if opa_process:
        opa_process.terminate()
        opa_process.wait()
        logger.info("OPA sidecar stopped")

def check_opa_circuit_breaker():
    """Check if OPA circuit breaker should block requests"""
    global opa_circuit_breaker
    
    current_time = time.time()
    
    # Reset failure count if window has passed
    if current_time - opa_circuit_breaker['last_failure'] > OPA_CIRCUIT_BREAKER_WINDOW:
        opa_circuit_breaker['failures'] = 0
        opa_circuit_breaker['state'] = 'closed'
    
    # Check if circuit breaker should open
    if opa_circuit_breaker['failures'] >= OPA_CIRCUIT_BREAKER_THRESHOLD:
        if opa_circuit_breaker['state'] == 'closed':
            logger.error(f"OPA circuit breaker OPENED due to {opa_circuit_breaker['failures']} failures")
            opa_circuit_breaker['state'] = 'open'
        return False  # Circuit is open
    
    return True  # Circuit is closed

def record_opa_failure():
    """Record OPA failure for circuit breaker"""
    global opa_circuit_breaker
    opa_circuit_breaker['failures'] += 1
    opa_circuit_breaker['last_failure'] = time.time()
    
    # Log security alert for OPA failures
    logger.error(f"OPA failure #{opa_circuit_breaker['failures']} recorded - security policy enforcement degraded")

def audit_security_event(event_type, user_id, customer_id, method, path, result, details=None):
    ...
    # Log to structured format for SIEM ingestion
    logger.info(f"SECURITY_AUDIT: {json.dumps(audit_data)}")
    
    # Store in Redis for real-time monitoring (optional)
    if r:
        try:
            r.lpush('security_audit_log', json.dumps(audit_data))
            r.ltrim('security_audit_log', 0, 9999)  # Keep last 10k events
        except Exception as e:
            logger.warning(f"Failed to store audit event: {e}")

def check_opa_policy(method, path, user_id, customer_id, headers=None):
    """Check request against OPA policies with circuit breaker and audit logging"""
    if not OPA_ENABLED:
        audit_security_event('opa_disabled', user_id, customer_id, method, path, 'allowed', 
                           {'reason': 'OPA disabled in configuration'})
        return True
    
    # Check circuit breaker
    if not check_opa_circuit_breaker():
        audit_security_event('opa_circuit_open', user_id, customer_id, method, path, 
                           'denied' if OPA_FAIL_CLOSED else 'allowed',
                           {'reason': 'Circuit breaker open', 'fail_closed': OPA_FAIL_CLOSED})
        return not OPA_FAIL_CLOSED  # Fail closed in production, open in dev
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Audit successful policy check
            audit_security_event('opa_policy_check', user_id, customer_id, method, path,
                               'allowed' if policy_result else 'denied',
                               {'opa_response': result})
            
            return policy_result
        else:
            record_opa_failure()
            audit_security_event('opa_error', user_id, customer_id, method, path,
                               'denied' if OPA_FAIL_CLOSED else 'allowed',
                               {'error': f'HTTP {response.status_code}', 'fail_closed': OPA_FAIL_CLOSED})
            return not OPA_FAIL_CLOSED
            
    except Exception as e:
        record_opa_failure()
        audit_security_event('opa_exception', user_id, customer_id, method, path,
                           'denied' if OPA_FAIL_CLOSED else 'allowed',
                           {'error': str(e), 'fail_closed': OPA_FAIL_CLOSED})
        return not OPA_FAIL_CLOSED

# Rate limiting configuration
limiter = Limiter(
    key_func=get_remote_address,
    default_limits=[f"{get_secret('API_RATE_LIMIT', '200')} per minute"]
)
limiter.init_app(app)

# Configuration
JWT_SECRET = get_secret('JWT_SECRET', 'default-jwt-secret-change-me')
WEBHOOK_SECRET = get_secret('WEBHOOK_SECRET', 'default-webhook-secret')
PAYMENT_PROVIDER_API_KEY = get_secret('PAYMENT_PROVIDER_API_KEY')
...
@app.before_request
def before_request():
    """Global request validation with OPA policy enforcement"""
    # Skip auth for health, webhook, and internal endpoints
    if request.endpoint in ['health', 'webhook', 'metrics', 'get_order_owner']:
        return
    ...
    if not check_opa_policy(
        ...
        # Detailed audit logging for denied requests
        audit_security_event('access_denied', request.user_id, request.customer_id, 
                           request.method, request.path, 'denied',
                           {'reason': 'OPA policy violation', 'user_agent': request.headers.get('User-Agent')})
        abort(403)
...
def create_payment_invoice(order_id: str, amount: float, currency: str = "USD") -> str:
    ...
    try:
        ...
        if PAYMENT_PROVIDER_API_KEY:
            ...
            # Use production payment handler
            logger.error("Payment provider API unavailable - using fallback")
...
@app.route('/internal/orders/<order_id>/owner')
def get_order_owner(order_id: str):
    """Internal endpoint for OPA ownership verification"""
    # Verify internal authentication
    internal_auth = request.headers.get('X-Internal-Auth')
    if internal_auth != 'opa-verification':
        abort(403)
    ...
    try:
        ...
        logger.error(f"Error verifying order ownership {order_id}: {e}")
...
if __name__ == '__main__':
    ...
    logger.info(f"OPA enabled: {OPA_ENABLED}")
    logger.info(f"OPA fail-closed mode: {OPA_FAIL_CLOSED}")
    logger.info(f"OPA circuit breaker threshold: {OPA_CIRCUIT_BREAKER_THRESHOLD}")
    
    # Start OPA sidecar if enabled
    if OPA_ENABLED:
        start_opa_sidecar()
        # Give OPA time to start up
        time.sleep(2)
