...
class RemoteAndroidManager:
    ...
    def _get_fly_app_ip(self) -> Optional[str]:
        ...
        try:
            # Prefer DNS or provided host
            import socket, shutil, json
            fly_hostname = os.getenv('FLY_ANDROID_HOST', f"{self.fly_app_name}.fly.dev")
            try:
                ip = socket.gethostbyname(fly_hostname)
                logger.info(f"Resolved {fly_hostname} to {ip}")
                return ip
            except socket.gaierror:
                logger.warning(f"DNS resolution failed for {fly_hostname}, trying flyctl")

            # Try to get IP from flyctl only if present
            if shutil.which('flyctl'):
                ...
                logger.info("flyctl not found; skipping flyctl IP lookup")
