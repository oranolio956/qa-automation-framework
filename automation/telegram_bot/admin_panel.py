...
Choose what data to export:
    ...
    async def system_management(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """System management and maintenance interface"""
        try:
...
**System Health:**
🖥️ CPU Usage: {stats.get('cpu_usage', 0):.1f}%
💾 Memory Usage: {stats.get('memory_usage', 0):.1f}%
💽 Disk Usage: {stats.get('disk_usage', 0):.1f}%
🌐 System Status: {"✅ Healthy" if stats.get('system_healthy', False) else "⚠️ Issues Detected"}

**Active Services:**
🤖 Telegram Bot: ✅ Running
📱 Snapchat Creator: ✅ Ready
...
Choose an operation to perform:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    ...
                    InlineKeyboardButton("🧹 Clear Cache", callback_data="system_clear_cache")
                ],
                [
                    InlineKeyboardButton("📊 Service Status", callback_data="system_status"),
                    InlineKeyboardButton("🔧 Maintenance Mode", callback_data="system_maintenance")
                ],
                [
                    InlineKeyboardButton("📋 View Logs", callback_data="system_logs"),
                    InlineKeyboardButton("🗄️ Database Health", callback_data="system_db_health")
                ],
                [
                    InlineKeyboardButton("⚡ Performance Test", callback_data="system_perf_test"),
                    InlineKeyboardButton("🔒 Security Check", callback_data="system_security")
                ],
                [
                    InlineKeyboardButton("◀️ Back to Dashboard", callback_data="admin_dashboard")
                ]
            ...
            logger.error(f"Error in system management: {e}")
            await update.callback_query.edit_message_text(
                "❌ Error accessing system management. Please check logs."
            )
...
- Disable new order processing
- Show maintenance message to users
- Allow admin access only

Current Status: ⚠️ Not in maintenance mode
            """
            
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("🔧 Enable Maintenance", callback_data="maintenance_enable"),
                    InlineKeyboardButton("✅ Disable Maintenance", callback_data="maintenance_disable")
                ],
                [
                    InlineKeyboardButton("◀️ Back to System", callback_data="admin_system")
                ]
            ])
    ...
    async def manage_payments(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
...
**Payment Operations:**
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("💰 View Transactions", callback_data="payments_transactions"),
                    InlineKeyboardButton("💸 Process Refund", callback_data="payments_refund")
                ],
                [
                    InlineKeyboardButton("📊 Payment Analytics", callback_data="payments_analytics"),
                    InlineKeyboardButton("🔍 Search Payment", callback_data="payments_search")
                ],
                [
                    InlineKeyboardButton("⚠️ Failed Payments", callback_data="payments_failed"),
                    InlineKeyboardButton("🔄 Retry Payment", callback_data="payments_retry")
                ],
                [
                    InlineKeyboardButton("◀️ Back to Dashboard", callback_data="admin_dashboard")
                ]
            ...
            logger.error(f"Error in payment management: {e}")
            await update.callback_query.edit_message_text(
                "❌ Error accessing payment management. Please check logs."
            )
